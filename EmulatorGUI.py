...
InterruptPins =[] #PNH bổ sung xử lý ngắt

GPIONames=["14","15","18","23","24","25","8","7","12","16","20","21","2","3","4","17","27","22","10","9","11","5","6","13","19","26"]

#PNH bổ sung xử lý ngắt
class InterruptData:
    def __init__(self, PinNumber, InterruptCondiction, CallBack, BounceTime):
        self.PinNumber=PinNumber
        self.InterruptCondiction=InterruptCondiction
...
def toggleButton(gpioID): 
    ...
    #PNH bổ sung xử lý sự kiện ngắt trên GPIO
...
class GPIO:
    ...
    def add_event_detect(channel, edge, callback, bouncetime=300):
        global InterruptPins 
        for b in InterruptPins: 
            if b.PinNumber == channel:
                return
        item = InterruptData(channel,edge, callback, bouncetime)
        InterruptPins.append(item)

    def remove_event_detect(channel):
        global InterruptPins 
        for b in InterruptPins: 
            if b.PinNumber == channel:
                InterruptPins.remove(b)
                return
