...
def toggle_auto_mode(channel):
    global auto_mode
    expected_channel = BTN_AUTO
    channel = int(str(channel))    
    if int(channel) == expected_channel:
...
def toggle_selection(channel):
    global selection
    expected_channel = BTN_SELECT
    channel = int(str(channel))  
    if int(channel) == expected_channel:
        selection = "HUMIDITY" if selection == "TEMPERATURE" else "TEMPERATURE"
...
def adjust_value(channel, increase=True):
    ...
    channel = int(str(channel)) 
    if int(channel) == BTN_UP or int(channel) == BTN_DOWN:  
...
def update_display(temp, humid):
    ...
    GPIO.output(LED_auto_mode, auto_mode)
